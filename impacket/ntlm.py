...
def getNTLMSSPType1(
    ...
    if signingRequired:
        auth["flags"] = (
            NTLMSSP_NEGOTIATE_KEY_EXCH | NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL
        )
    # Set NTLMSSP Negotiate Flags to match Windows client observed in Wireshark (0xe2088297)
    if use_ntlmv2:
...
def computeResponseNTLMv2(
    ...
    av_pairs[NTLMSSP_AV_TARGET_NAME] = (
        f"{service}/".encode("utf-16le") + av_pairs[NTLMSSP_AV_DNS_HOSTNAME][1]
    )
