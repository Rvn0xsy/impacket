...
class SMBConnection:
    ...
    def __init__(
        ...
        if existingConnection is not None:
            ...
            assert isinstance(existingConnection, smb.SMB) or isinstance(
                existingConnection, smb3.SMB3
            )
    ...
    def negotiateSession(
        ...
        if self._sess_port == nmb.SMB_SESSION_PORT and self._remoteName == "*SMBSERVER":
            ...
            self._sess_port == nmb.NETBIOS_SESSION_PORT
            and self._remoteName == "*SMBSERVER"
        ):
    ...
    def login(self, user, password, domain="", lmhash="", nthash="", ntlmFallback=True):
        ...
        try:
            if self.getDialect() == smb.SMB_DIALECT:
                return self._SMBConnection.login(
    ...
    def createFile(
        ...
        impersonationLevel=smb3structs.SMB2_IL_IMPERSONATION,
        securityFlags=0,
        oplockLevel=smb3structs.SMB2_OPLOCK_LEVEL_NONE,
        createContexts=None,
    ):
        """
        ...
        if self.getDialect() == smb.SMB_DIALECT:
            ...
            try:
                return self._SMBConnection.nt_create_andx(
                    treeId, pathName, cmd=ntCreate
                )
            except (smb.SessionError, smb3.SessionError) as e:
                raise SessionError(e.get_error_code(), e.get_error_packet())
        else:
    ...
    def readFile(self, treeId, fileId, offset=0, bytesToRead=None, singleCall=True):
        ...
        while not finished:
            ...
            try:
                bytesRead = self._SMBConnection.read_andx(
                    treeId, fileId, offset, toRead
                )
            except (smb.SessionError, smb3.SessionError) as e:
    ...
    def getFile(
        ...
        """
        try:
            return self._SMBConnection.retr_file(
                shareName,
                pathName,
                callback,
                mode=mode,
    ...
    def putFile(
        ...
        """
        try:
            return self._SMBConnection.stor_file(
                shareName,
                pathName,
                callback,
                mode=mode,
        ...
        try:
            if self.getDialect() == smb.SMB_DIALECT:
                if not fileInfoClass:
                    ...
                    res = self._SMBConnection.query_file_info(
                        treeId, fileId, fileInfoClass=fileInfoClass
                    )
            else:
                if not fileInfoClass:
                    ...
                    res = self._SMBConnection.queryInfo(
                        treeId, fileId, fileInfoClass=fileInfoClass
                    )
            return res
        except (smb.SessionError, smb3.SessionError) as e:
    ...
    def setInfo(self, treeId, fileId, fileInfoClass, infoData):
        ...
        try:
            if self.getDialect() == smb.SMB_DIALECT:
                return self._SMBConnection.set_file_info(
                    treeId, fileId, fileInfoClass=fileInfoClass, file_info_data=infoData
                )
            else:
    ...
    def transactNamedPipe(self, treeId, fileId, data, waitAnswer=True):
        ...
        """
        try:
            return self._SMBConnection.TransactNamedPipe(
                treeId, fileId, data, waitAnswer=waitAnswer
            )
        except (smb.SessionError, smb3.SessionError) as e:
            raise SessionError(e.get_error_code(), e.get_error_packet())
    ...
    def readNamedPipe(self, treeId, fileId, bytesToRead=None):
        ...
        try:
            return self.readFile(
                treeId, fileId, bytesToRead=bytesToRead, singleCall=True
            )
        except (smb.SessionError, smb3.SessionError) as e:
            raise SessionError(e.get_error_code(), e.get_error_packet())
    ...
    def listSnapshots(self, tid, path):
        ...
        try:
            snapshotData = smb3structs.SRV_SNAPSHOT_ARRAY(
                self._SMBConnection.ioctl(
                    tid,
                    fid,
                    smb3structs.FSCTL_SRV_ENUMERATE_SNAPSHOTS,
        ...
        self.closeFile(tid, fid)
        return list(
            filter(None, snapshotData["SnapShots"].decode("utf16").split("\x00"))
        )
    ...
    def createMountPoint(self, tid, path, target):
        ...
        self._SMBConnection.ioctl(
            tid,
            fid,
            smb3structs.FSCTL_SET_REPARSE_POINT,
            flags=smb3structs.SMB2_0_IOCTL_IS_FSCTL,
    ...
    def removeMountPoint(self, tid, path):
        ...
        try:
            self._SMBConnection.ioctl(
                tid,
                fid,
                smb3structs.FSCTL_DELETE_REPARSE_POINT,
                flags=smb3structs.SMB2_0_IOCTL_IS_FSCTL,
                inputBlob=reparseData,
            )
        except (smb.SessionError, smb3.SessionError) as e:
    ...
    def setHostnameValidation(self, validate, accept_empty, hostname):
        return self._SMBConnection.set_hostname_validation(
            validate, accept_empty, hostname
        )
...
class SessionError(Exception):
    ...
    def __str__(self):
        ...
        if key in nt_errors.ERROR_MESSAGES:
            ...
            return "SMB SessionError: code: 0x%x - %s - %s" % (
                self.error,
                error_msg_short,
                error_msg_verbose,
            )
