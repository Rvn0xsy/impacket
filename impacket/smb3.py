...
class SMB3:
    ...
    def processContextList(self, contextCount, contextList):
        ...
        while contextCount > 0:
            ...
            if context["ContextType"] == SMB2_PREAUTH_INTEGRITY_CAPABILITIES:
                ...
                self._Connection["PreauthIntegrityHashId"] = struct.unpack(
                    "<H", contextPreAuth["HashAlgorithms"]
                )[0]
    ...
    def signSMB(self, packet):
        ...
        if (
            self._Connection["Dialect"] == SMB2_DIALECT_21
            or self._Connection["Dialect"] == SMB2_DIALECT_002
        ):
            if len(self._Session["SessionKey"]) > 0:
                signature = hmac.new(
                    self._Session["SessionKey"], packet.getData(), hashlib.sha256
                ).digest()
    ...
    def sendSMB(self, packet):
        ...
        if (
            self._Session["SigningActivated"] is True
            and self._Connection["SequenceWindow"] > 2
        ):
            if (
                packet["TreeID"] > 0
                and (packet["TreeID"] in self._Session["TreeConnectTable"]) is True
            ):
                if (
        ...
        if (self._Session["SessionFlags"] & SMB2_SESSION_FLAG_ENCRYPT_DATA) or (
            packet["TreeID"] != 0
            and self._Session["TreeConnectTable"][packet["TreeID"]]["EncryptData"]
            is True
        ):
    ...
    def negotiateSession(self, preferredDialect=None, negSessionResponse=None):
        ...
        ) == SMB2_NEGOTIATE_SIGNING_REQUIRED or self._Connection[
            "Dialect"
        ] == SMB2_DIALECT_311:
        ...
        if self._Connection["Dialect"] == SMB2_DIALECT_311:
            ...
            if negContextCount > 0:
                self.processContextList(
                    negContextCount, negResp["NegotiateContextList"]
                )

        if (
            negResp["Capabilities"] & SMB2_GLOBAL_CAP_LEASING
        ) == SMB2_GLOBAL_CAP_LEASING:
            self._Connection["SupportsFileLeasing"] = True
        if (
            negResp["Capabilities"] & SMB2_GLOBAL_CAP_LARGE_MTU
        ) == SMB2_GLOBAL_CAP_LARGE_MTU:
        ...
        if self._Connection["Dialect"] >= SMB2_DIALECT_30:
            ...
            if (
                negResp["Capabilities"] & SMB2_GLOBAL_CAP_DIRECTORY_LEASING
            ) == SMB2_GLOBAL_CAP_DIRECTORY_LEASING:
                self._Connection["SupportsDirectoryLeasing"] = True
            if (
                negResp["Capabilities"] & SMB2_GLOBAL_CAP_MULTI_CHANNEL
            ) == SMB2_GLOBAL_CAP_MULTI_CHANNEL:
                self._Connection["SupportsMultiChannel"] = True
            if (
                negResp["Capabilities"] & SMB2_GLOBAL_CAP_PERSISTENT_HANDLES
            ) == SMB2_GLOBAL_CAP_PERSISTENT_HANDLES:
                self._Connection["SupportsPersistentHandles"] = True
            if (
                negResp["Capabilities"] & SMB2_GLOBAL_CAP_ENCRYPTION
            ) == SMB2_GLOBAL_CAP_ENCRYPTION:
                self._Connection["SupportsEncryption"] = True
    ...
    def kerberosLogin(
        ...
        if TGS is None:
            serverName = Principal(
                "cifs/%s" % (self._Connection["ServerName"]),
        ...
        self._Session["PreauthIntegrityHashValue"] = self._Connection[
        ...
        if ans.isValidAnswer(STATUS_SUCCESS):
            ...
            if mutualAuth == True:
                ...
                apSessionKey = Key(
                    apCipher.enctype, encAPRepPart["subkey"]["keyvalue"].asOctets()
                )
            ...
            if (
                self._Session["SigningRequired"] is True
                and self._Connection["Dialect"] >= SMB2_DIALECT_30
            ):
        ...
        if ans.isValidAnswer(STATUS_MORE_PROCESSING_REQUIRED):
            ...
            if ntlmChallenge["TargetInfoFields_len"] > 0:
                ...
                if av_pairs[ntlm.NTLMSSP_AV_HOSTNAME] is not None:
                    try:
                        self._Session["ServerName"] = av_pairs[
                ...
                if av_pairs[ntlm.NTLMSSP_AV_DOMAINNAME] is not None:
                    try:
                        if self._Session["ServerName"] != av_pairs[
                            ntlm.NTLMSSP_AV_DOMAINNAME
                        ][1].decode("utf-16le"):
                            self._Session["ServerDomain"] = av_pairs[
                                ntlm.NTLMSSP_AV_DOMAINNAME
                            ][1].decode("utf-16le")
                    except:
                        # For some reason, we couldn't decode Unicode here.. silently discard the operation
                ...
                if av_pairs[ntlm.NTLMSSP_AV_DNS_HOSTNAME] is not None:
                    try:
                        self._Session["ServerDNSHostName"] = av_pairs[
                ...
                if "Version" in ntlmChallenge.fields:
                    ...
                    if len(version) >= 4:
                        if struct.unpack("<H", version[2:4])[0] in WIN_VERSIONS.keys():
                            self._Session["ServerOS"] = (
                                WIN_VERSIONS[struct.unpack("<H", version[2:4])[0]]
                                + " Build %d" % struct.unpack("<H", version[2:4])[0]
                            )
                        else:
                        ...
                        self._Session["ServerOSBuild"] = struct.unpack(
                            "<H", version[2:4]
                        )[0]
    ...
    def connectTree(self, share):
        ...
        if share in self._Session["TreeConnectTable"]:
            ...
            self._Session["TreeConnectTable"][treeEntry["TreeConnectId"]][
                "NumberOfUses"
            ] += 1
        ...
        if packet.isValidAnswer(STATUS_SUCCESS):
            ...
            treeEntry["NumberOfUses"] += 1
            if (
                treeConnectResponse["Capabilities"] & SMB2_SHARE_CAP_DFS
            ) == SMB2_SHARE_CAP_DFS:
                treeEntry["IsDfsShare"] = True
            if (
                treeConnectResponse["Capabilities"]
    ...
    def disconnectTree(self, treeId):
        ...
        if treeId in self._Session["TreeConnectTable"]:
            ...
            if self._Session["TreeConnectTable"][treeId]["NumberOfUses"] > 1:
                ...
                treeEntry["NumberOfUses"] -= 1
                self._Session["TreeConnectTable"][treeEntry["ShareName"]][
                    "NumberOfUses"
                ] -= 1
                return True
    ...
    def create(
        ...
        if (
            self._Connection["Dialect"] >= SMB2_DIALECT_30
            and self._Connection["SupportsDirectoryLeasing"] is True
        ):
        ...
        if createContexts is not None:
            ...
            smb2Create["CreateContextsOffset"] = (
                len(SMB2Packet()) + SMB2Create.SIZE + len(smb2Create["Buffer"])
            )

            # pad offset to 8-byte align
            if smb2Create["CreateContextsOffset"] % 8:
                smb2Create["Buffer"] += b"\x00" * (
                    8 - (smb2Create["CreateContextsOffset"] % 8)
                )
                smb2Create["CreateContextsOffset"] = (
                    len(SMB2Packet()) + SMB2Create.SIZE + len(smb2Create["Buffer"])
                )
    ...
    def read(self, treeId, fileId, offset=0, bytesToRead=0, waitAnswer=True):
        ...
        if (
            ...
            and self._Connection["SupportsMultiCredit"] is True
        ):
    ...
    def timestampForSnapshot(self, path):
        ...
        fTime = int(
            (
                datetime.strptime(timestamp, "@GMT-%Y.%m.%d-%H.%M.%S")
                - datetime(1970, 1, 1)
            ).total_seconds()
        )
        fTime *= 10000000
        ...
        try:
            ...
            while True:
                try:
                    ...
                    nextOffset = 1
                    while nextOffset != 0:
                        fileInfo = smb.SMBFindFileFullDirectoryInfo(
        ...
        try:
            ...
            if (fileSize - offset) < self._Connection["MaxReadSize"]:
                ...
                while written < toBeRead:
                    data = self.read(
                        treeId, fileId, offset, self._Connection["MaxReadSize"]
                    )
                    written += len(data)
                    offset += len(data)
                    callback(data)
    ...
    def read_andx(
        self, tid, fid, offset=0, max_size=None, wait_answer=1, smb_packet=None
    ):
        # ToDo: Handle the custom smb_packet situation
        if max_size is None:
    ...
    def perform_hostname_validation(self):
        ...
        if (
            ...
            and self._Session["ServerDNSHostName"].lower()
