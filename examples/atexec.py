...
CODEC = sys.stdout.encoding or "utf-8"  # Fix encoding when stdout.encoding is None
    ...
    <Exec>
      ...
      <Arguments>%s</Arguments>
        ...
        """ % (
            (
                xml_escape(cmd)
                if self.__silentCommand is False
                else self.__command.split()[0]
            ),
            (
                xml_escape(args)
                if self.__silentCommand is False
                else " ".join(self.__command.split()[1:])
            ),
        )
        taskCreated = False
        try:
            ...
            if self.sessionId is None:
                ...
                try:
                    tsch.hSchRpcRun(
                        dce,
                        "\\%s" % tmpName,
                        flags=tsch.TASK_RUN_USE_SESSION_ID,
                        sessionId=self.sessionId,
                    )
                except Exception as e:
                    if (
                        str(e).find("ERROR_FILE_NOT_FOUND") >= 0
                        or str(e).find("E_INVALIDARG") >= 0
                    ):
        ...
        while True:
            try:
                ...
                smbConnection.getFile(
                    "ADMIN$", "Temp\\%s" % tmpFileName, output_callback
                )
                break
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=" ",
        help="command to execute at the target ",
    )
    parser.add_argument(
        "-session-id",
        action="store",
        type=int,
        help="an existed logon session to use (no output, no cmd.exe)",
    )
    parser.add_argument(
